    ...
    <section className="card overflow-hidden">
      <div className="flex items-start justify-between gap-3 border-b border-pl-border px-5 py-3">
        <div>
          <p className="eyebrow text-pl-pink">
            <span className="whitespace-nowrap">Koch of the week</span>
            {!confirmed && <span className="block font-normal sm:inline"> (so far)*</span>}
          </p>
          ...
          return (
            <div key={key} className="flex gap-4 bg-pl-surface p-5">
              ...
              <div className="flex min-w-0 flex-1 flex-col justify-center">
                ...
                {!confirmed && gameweek.fixturesRemaining && (
                  <p className="tnum mt-2 text-sm text-pl-muted">{gameweek.yetByManager?.[key] ?? 0} still to play</p>
                )}
              </div>
            </div>
          )
