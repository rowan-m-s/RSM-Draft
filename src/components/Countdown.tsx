...
function Unit({ value, label, urgent }: { value: number; label: string; urgent: boolean }) {
  return (
    <div className="flex flex-col items-center">
      <span
        className={`display tnum text-2xl leading-none sm:text-3xl ${urgent ? 'text-pl-pink' : 'text-pl-navy'}`}
      >
...
function Clock({ target, now }: { target: string; now: Date }) {
  const { days, hours, minutes, seconds, urgent } = remainingUntil(target, now)
  return (
    <div className="flex items-start gap-3 sm:gap-4">
      <Unit value={days} label="days" urgent={urgent} />
      <Unit value={hours} label="hrs" urgent={urgent} />
      <Unit value={minutes} label="min" urgent={urgent} />
      <Unit value={seconds} label="sec" urgent={urgent} />
    </div>
  )
}
...
export function DeadlineStrip({ gameweeks }: { gameweeks: Gameweek[] }) {
  ...
  if (!deadline) {
    return (
      <section className="card p-5">
        ...
        <p className="display mt-2 text-2xl text-pl-navy">That's the season</p>
  ...
  return (
    <section
      className={`card overflow-hidden ${urgent ? 'ring-2 ring-pl-pink' : ''}`}
    ...
    >
      <div className="grid gap-px bg-pl-border sm:grid-cols-2">
        {waiver && (
          <div className="bg-pl-white p-5">
            <div className="flex items-baseline justify-between gap-2">
              <p className="eyebrow text-pl-cyan-ink">Waiver deadline</p>
        ...
        <div className="bg-pl-white p-5">
          <div className="flex items-baseline justify-between gap-2">
            <p className={`eyebrow ${urgent ? 'text-pl-pink' : 'text-pl-muted'}`}>
